...
void loop() {
  ...
  if(needleMove == "up" || needleMove == "down") {
    moveNeedles(needleMove);
    needleMove = "none";
  }
  /*
...
void drawerMove() {
  ...
  if (interrupt_time - last_interrupt_time > switchDebounceTime) {
    int rising1 = digitalRead(drawerPin);
    delay(2);
    int rising2 = digitalRead(drawerPin);
    delay(2);
    int rising3 = digitalRead(drawerPin);
    delay(2);
    int rising4 = digitalRead(drawerPin);
    delay(2);
    int rising5 = digitalRead(drawerPin);
    int rising = 0;
    if((rising1 + rising2 + rising3 + rising4 + rising5) >= 3) { rising = 1; }
